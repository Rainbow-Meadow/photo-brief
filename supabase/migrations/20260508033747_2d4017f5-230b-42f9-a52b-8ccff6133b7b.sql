...
-- Platform admin access policies (using is_platform_admin() security definer function)
CREATE POLICY business_intake_profiles_platform_admin_all ON public.business_intake_profiles FOR ALL USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());
CREATE POLICY website_scan_jobs_platform_admin_all ON public.website_scan_jobs FOR ALL USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());
CREATE POLICY website_pages_platform_admin_all ON public.website_pages FOR ALL USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());
CREATE POLICY website_forms_platform_admin_all ON public.website_forms FOR ALL USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());
CREATE POLICY service_catalog_items_platform_admin_all ON public.service_catalog_items FOR ALL USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());
CREATE POLICY intake_blueprints_platform_admin_all ON public.intake_blueprints FOR ALL USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());
CREATE POLICY intake_routing_rules_platform_admin_all ON public.intake_routing_rules FOR ALL USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());
CREATE POLICY website_change_events_platform_admin_all ON public.website_change_events FOR ALL USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());