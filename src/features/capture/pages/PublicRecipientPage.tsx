    ...
    >
      <div className="space-y-4">
        <div className="space-y-1">
          ...
          <ReadinessProgress value={flow.progress.total === 0 ? 0 : (flow.progress.done / flow.progress.total) * 100} />
        </div>
        ...
        <ChatThread autoScrollKey={flow.messages.length}>
          {flow.messages.map((msg) => {
            switch (msg.kind) {
              case "assistant_text":
                return <ChatMessage key={msg.id} from="assistant"><p className="text-sm">{msg.text}</p></ChatMessage>;
              case "user_text":
                return <ChatMessage key={msg.id} from="user"><p className="text-sm">{msg.text}</p></ChatMessage>;
              case "photo_prompt":
                return <ChatMessage key={msg.id} from="assistant"><PhotoPromptCard step={msg.step} index={msg.index} total={msg.total} /></ChatMessage>;
              case "capture_card":
                return <ChatMessage key={msg.id} from="assistant" bare><CaptureUploadCard step={msg.step} pending={msg.pending} onCapture={flow.submitPhoto} /></ChatMessage>;
              case "user_photo":
                return <ChatMessage key={msg.id} from="user" bare><img src={msg.photo.previewUrl} alt="Submitted" className="block max-h-72 w-full object-cover" /></ChatMessage>;
              case "ai_feedback":
                return <ChatMessage key={msg.id} from="assistant"><AIFeedbackMessage photo={msg.photo} verdict={msg.verdict} /></ChatMessage>;
              case "retake_decision":
                return <ChatMessage key={msg.id} from="assistant"><RetakeDecisionCard photo={msg.photo} step={msg.step} onRetake={flow.retake} onUseAnyway={flow.useAnyway} /></ChatMessage>;
              case "question":
                return <ChatMessage key={msg.id} from="assistant"><QuestionCard question={msg.question} onAnswer={flow.answerQuestion} /></ChatMessage>;
              case "review_summary":
                return <ChatMessage key={msg.id} from="assistant" bare><div className="p-4"><ReviewSummaryCard guide={ctx.guide} photos={flow.photos} answers={flow.answers} onSubmit={handleSubmit} /></div></ChatMessage>;
              case "submit_confirmation":
                return <ChatMessage key={msg.id} from="assistant" bare><div className="p-4"><SubmitConfirmationCard businessName={ctx.businessName} /></div></ChatMessage>;
              default:
                return null;
            }
          })}
