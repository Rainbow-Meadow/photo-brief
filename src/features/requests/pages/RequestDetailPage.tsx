      ...
      <section className="surface-card p-5">
        ...
        {messages.length === 0 ? (
          <p className="mt-2 text-sm text-muted-foreground">
            No messages sent yet. Copy the text above, send it from your phone, then mark it as sent.
          </p>
        ) : (
          <ul className="mt-3 space-y-2">
            {messages.map((m) => {
              const inbound = m.direction === "inbound";
              const isManual = (m.metadata as { delivery?: string; source?: string })?.delivery === "manual";
              return (
                <li
                  key={m.id}
                ...
                >
                  <div className="min-w-0">
                    <p className="font-medium capitalize text-foreground">
                      {inbound ? "Reply" : isManual ? "Manual text" : m.kind}{" "}
