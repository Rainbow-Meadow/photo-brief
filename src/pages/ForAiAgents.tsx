    ...
    <>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-4xl px-4 pt-14 text-center sm:px-6 sm:pt-20 lg:px-8">
          ...
          <h1 className="text-display mt-6 text-foreground">PhotoBrief is a visual intake layer.</h1>
          ...
          <div className="mt-8 grid gap-3 text-left sm:grid-cols-2">
            <div className="glass-strong rounded-2xl p-4">
              ...
              <p className="mt-3 text-sm font-semibold text-foreground">Website Intake</p>
              <p className="mt-1 text-xs leading-5 text-muted-foreground">Hosted form or webhook turns website leads into PhotoBrief requests.</p>
            </div>
            <div className="glass-strong rounded-2xl p-4">
              <Route className="h-5 w-5 text-primary" />
              <p className="mt-3 text-sm font-semibold text-foreground">Template routing</p>
              <p className="mt-1 text-xs leading-5 text-muted-foreground">Rules and conservative AI fallback select from configured templates only.</p>
            </div>
          </div>
          <div className="h-12" />
        </div>
          ...
          <div className="mt-10 overflow-hidden rounded-2xl border bg-card">
            <Tabs value={lang} onValueChange={(v) => setLang(v as ApiExampleLang)}>
              <TabsList className="w-full justify-start rounded-none border-b bg-muted/30 px-2">
                <TabsTrigger value="curl" className="gap-1.5"><Terminal className="h-3.5 w-3.5" /> cURL</TabsTrigger>
                <TabsTrigger value="javascript" className="gap-1.5"><Code2 className="h-3.5 w-3.5" /> JavaScript</TabsTrigger>
                <TabsTrigger value="python" className="gap-1.5"><Code2 className="h-3.5 w-3.5" /> Python</TabsTrigger>
              </TabsList>
              {(Object.keys(API_EXAMPLES) as ApiExampleLang[]).map((l) => (
                <TabsContent key={l} value={l} className="m-0">
                  <pre className="overflow-x-auto p-5 text-xs leading-relaxed text-foreground"><code>{API_EXAMPLES[l]}</code></pre>
                </TabsContent>
              ))}
            </Tabs>
          </div>
...
{`{
  ...
  "recipient_url": "https://photobrief.ai/r/abc123"
              ...
              <p className="mt-3 text-muted-foreground">Forward <code>recipient_url</code> to the customer, or use Website Intake to send automatically.</p>
            </article>
            <article className="hairline rounded-2xl bg-card p-5 text-sm">
      ...
      <section id="mcp" aria-labelledby="mcp-heading" className="bg-muted/20">
        <div className="mx-auto max-w-5xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-4 text-base text-muted-foreground">Static descriptors point your agent at the right endpoints and capabilities.</p>
          </div>
          ...
          <div className="mt-10 grid gap-4 sm:grid-cols-2">
            <article className="hairline rounded-2xl bg-card p-5">
              ...
              <p className="mt-2 text-sm text-muted-foreground">Capabilities, auth, and discovery URLs in one file.</p>
              <pre className="mt-3 overflow-x-auto rounded bg-muted p-3 text-xs">{`GET https://photobrief.ai/.well-known/agent.json`}</pre>
            </article>
            <article className="hairline rounded-2xl bg-card p-5">
              <h3 className="text-sm font-semibold text-foreground">mcp.json</h3>
              <p className="mt-2 text-sm text-muted-foreground">MCP server descriptor with planned tools and a REST fallback today.</p>
              <pre className="mt-3 overflow-x-auto rounded bg-muted p-3 text-xs">{`GET https://photobrief.ai/mcp.json`}</pre>
            </article>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-5xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
          ...
          <ul className="mt-10 grid gap-3 sm:grid-cols-2">
            {DISCOVERY_LINKS.map((d) => (
              <li key={d.href}>
                <a href={d.href} className="hairline block rounded-2xl bg-card p-4 text-sm transition hover:bg-accent/40">
      ...
      <section id="faq" aria-labelledby="faq-heading" className="bg-muted/20">
        <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
          <div className="text-center">
            ...
            <p className="mt-4 text-sm text-muted-foreground">Same source as <NavLink to="/help" className="text-primary hover:underline">/help</NavLink>.</p>
          </div>
